...
module.exports = (graphqlOrigin) => {
  ...
  const assetsBase = `${process.env.ASSETS_S3_BUCKET}/${projectName}/assets` || path.resolve('build/assets');;
  console.log(assetsBase)
  return (req, res) => {
    if (req.body && req.body.id && !req.body.query) {
      try {
        if (!queryMap.get(req.body.id)) {
          const queryText = fs.readFileSync(`${assetsBase}/${req.body.id}.query`);
